...
window.MiaoControlHost.registerModule("broadcast", async () => {
  const messageInput = document.getElementById("broadcast-message");
  const statsElement = document.getElementById("broadcast-stats");
  const currentSongElement = document.getElementById("broadcast-current-song");
  const transmissionSettings = document.getElementById("broadcast-transmission-settings");
  const displaySettings = document.getElementById("broadcast-display-settings");
  const playerActions = document.getElementById("broadcast-player-actions");
  const shortcutList = document.getElementById("broadcast-shortcut-list");
  ...
  function setStatus(message, type = "") {
    window.MiaoControlHost.setStatus(message, type);
  }
  ...
  function createStandardField(field) {
    ...
    label.htmlFor = `broadcast-setting-${field.key}`;
    ...
    input.id = `broadcast-setting-${field.key}`;
  ...
  function getSettingInputs() {
    return Array.from(document.querySelectorAll(
      '[data-module="broadcast"] [data-setting]'
    ));
  }
  ...
  async function loadApplication() {
    ...
    setStatus("Module Broadcast connecté.", "ok");
  }
  ...
  document.getElementById("broadcast-save-message").addEventListener("click", queueMessageSave);
  document.getElementById("broadcast-save-settings").addEventListener("click", queueSettingsSave);

  document.getElementById("broadcast-reload").addEventListener("click", async () => {
    if (!applicationReady) {
      setStatus("La console n’est pas encore prête.", "error");
      return;
    }
  ...
  document.getElementById("broadcast-reset-settings").addEventListener("click", async () => {
    if (!applicationReady || !schema) {
      setStatus("La configuration n’est pas encore chargée.", "error");
      return;
    }
